...
final class FriendRepository: FriendRepositoryProtocol {
    ...
    func fetchFriendList1() async throws -> [Friend] {
        try await fetch(remote: { FriendMapper.map(try await self.remoteDataSource.fetchFriendList1()) }, cacheKey: .friend1)
    }

    func fetchFriendList2() async throws -> [Friend] {
        try await fetch(remote: { FriendMapper.map(try await self.remoteDataSource.fetchFriendList2()) }, cacheKey: .friend2)
    }

    func fetchFriendList3() async throws -> [Friend] {
        try await fetch(remote: { FriendMapper.map(try await self.remoteDataSource.fetchFriendList3()) }, cacheKey: .friend3)
    }

    func fetchFriendList4() async throws -> [Friend] {
        try await fetch(remote: { FriendMapper.map(try await self.remoteDataSource.fetchFriendList4()) }, cacheKey: .friend4)
    }
